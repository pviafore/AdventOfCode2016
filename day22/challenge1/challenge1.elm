...
readLines = (String.split "\n" input)
            |> List.drop 2

nodeRegex : Regex.Regex
nodeRegex = Regex.regex "/dev/grid/node-x(\\d+)-y(\\d+)\\s*(\\d+)T\\s*(\\d+)T\\s*(\\d+)T.*"

type alias Node = {x: Int, y: Int, size: Int, used: Int, avail:Int}

defaultNode : Node
defaultNode =  {x=1, y=1, size=1,used=1, avail=1}

unwrap : Maybe String -> Int
unwrap str = String.toInt (Maybe.withDefault "1" str) 
            |> Result.toMaybe 
            |> Maybe.withDefault 0  

regexToNode : Regex.Match -> Node
regexToNode match = 
    case (match.submatches) of 
      [x, y, size,used, avail] -> {x=(unwrap x), y=(unwrap y), size=(unwrap size), used=(unwrap used), avail=(unwrap avail)}
      otherwise -> defaultNode

stringToNode : String -> Node
stringToNode str = Maybe.withDefault defaultNode (Maybe.map regexToNode (findMatch str))

findMatch : String -> Maybe Regex.Match
findMatch str = (List.head (Regex.find Regex.All nodeRegex str))
...
notEmpty : Node -> Bool
notEmpty node = node.used /= 0

wouldFit : Node -> Node -> Bool
wouldFit node1 node2 = node1.used <= node2.avail

isViablePair : (Node, Node) -> Bool
isViablePair nodeTuple  = 
             let (node1, node2) = nodeTuple in
             node1 /= node2 && (notEmpty node1) && (wouldFit node1 node2)

product : List Node -> (List (Node, Node))
product nodes = List.concatMap (\node1 -> List.map (\node2 -> (node1, node2)) nodes) nodes

solve : List (Node, Node) -> Int
solve nodes = 
    nodes
    |> List.filter isViablePair
    |> List.length


main : Html.Html msg
main = text (toString ( solve (product readNodes)))